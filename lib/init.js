...
function initComplete(app, isInstall) {
    chalk.green.bold(`Success! Created ${app} project complete!`);
    if (isInstall) {
        chalk.yellow.bold(`begin by typing:
...
function init(app) {
    ...
    if (fs.existsSync(appDir)) {
        inquirer.prompt([{
            ...
            if (answer.appexist) {
                spinner.start(`removeing ${app} dir`);
                fs
                    .emptyDir(appDir)
                    .then(() => {
                        spinner.stop();
...
function downloadTemplate(app, answer) {
    ...
    switch (answer.templatetype) {
        case 'gulp':
            ...
            break;
        case 'webpack4.0':
            templateUrl = 'gkf442573575/wepack4.0-simple'
        default:
            templateUrl = 'gkf442573575/gulpsimple'
            break;
    }
